    ...
    public class TagsController : Controller
    {
        ...
        public ActionResult Create(Tag data)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} tag created {data.Name}.");
                    return Json(new
                    {
                        success = true,
        ...
        public ActionResult Edit(Tag tag)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (TempData["tagID"] != null && Int64.TryParse(TempData["tagID"].ToString(), out Id) && tag.ID == Id)
                {
                    ...
                    {
                        log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} tag updated {tag.Name}.");
                        return Json(new
                        {
                            success = true,
                            ...
                            data = new
                            {
                                Date = tag.CreatedOn.Value.ToString(CustomHelper.GetDateFormat),
                                Name = tag.Name,
        ...
        public ActionResult DeleteConfirmed(long id)
        {
            ...
            {
                Tag tag = _tagsService.GetTag((Int16)id);
                log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} tag deleted {tag.Name}.");
        ...
        public ActionResult Activate(long? id)
        {
            ...
            if (_tagsService.UpdateTag(ref tag, ref message))
            {
				...
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} {((bool)tag.IsActive ? "activated" : "deactivated")} {tag.Name}.");
                return Json(new
                {
                    success = true,
