...
namespace Project.Web.Areas.Admin.Controllers
{
	...
	public class AreaController : Controller
	{
		...
		[HttpGet]
		public ActionResult GetAreaByCity(long id)
		{
			var cities = _areaService.GetAreasForDropDown(id);
		...
		public ActionResult Create(Area area)
		{
			...
			if (ModelState.IsValid)
			{
				if (_areaService.CreateArea(area, ref message))
				{
					...
					log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} created area {area.Name}.");
					return Json(new
					{
						success = true,
		...
		public ActionResult Edit(long? id)
		{
			...
			ViewBag.CityID = new SelectList(_cityService.GetCitiesForDropDown((long)area.CountryID), "value", "text", area.CityID);
			...
			if (ModelState.IsValid)
			{
				...
				if (TempData["AreaID"] != null && Int64.TryParse(TempData["AreaID"].ToString(), out Id) && area.ID == Id)
				{
					if (_areaService.UpdateArea(ref area, ref message))
					{
						var city = _cityService.GetCity((long)area.CityID);
						var country = _countryService.GetCountry((long)area.CountryID);
						log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} updated area {area.Name}.");
						return Json(new
						{
							success = true,
							url = "/Admin/Area/Index",
		...
		public ActionResult Activate(long? id)
		{
			...
			{	
				...
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} {((bool)area.IsActive ? "activated" : "deactivated")} {area.Name}.");
				return Json(new
				{
					success = true,
		...
		public ActionResult DeleteConfirmed(long id)
		{
			...
			{
				Area area = _areaService.GetArea((Int16)id);
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} deleted area {area.Name}.");
                ...
                if (FileUpload.ContentType == "application/vnd.ms-excel" || FileUpload.ContentType == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                {
                    ...
                    if (filename.EndsWith(".xlsx"))
                    {
                        ...
                        try
                        {
							...
							log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} bulk uploaded area.");
