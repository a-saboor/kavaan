	...
	public class PaymentMethodController : Controller
	{
		...
		public ActionResult Create(PaymentMethod paymentmethod)
		{
			...
			if (ModelState.IsValid)
			{
				...
				{
						log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} created payment methods.");
					return Json(new
					{
						success = true,
		...
		public ActionResult Edit(PaymentMethod paymentmethod)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (TempData["PaymentMethodID"] != null && Int64.TryParse(TempData["PaymentMethodID"].ToString(), out Id) && paymentmethod.ID == Id)
				{
					...
					{
						log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} updated payment methods.");
						return Json(new
						{
							success = true,
							url = "/Admin/PaymentMethod/Index",
		...
		public ActionResult Activate(long? id)
		{
			...
			if (!(bool)paymentmethod.IsActive)
			{
				...
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} activated payment methods.");
			}
			else
			{
				paymentmethod.IsActive = false;
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} deactivated payment methods.");
			}
			string message = string.Empty;
			if (_paymentmethodService.UpdatePaymentMethod(ref paymentmethod, ref message))
			{
		...
		public ActionResult DeleteConfirmed(long id)
		{
			...
			{
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} deleted payment methods ID: {id}.");
