	...
	public class TaxSettingController : Controller
	{
		...
		public ActionResult Create(TaxSetting taxsetting)
		{
			...
			if (ModelState.IsValid)
			{
				...
				{
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} created tax settings.");
					return Json(new
					{
						success = true,
		...
		public ActionResult Edit(TaxSetting taxsetting)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (TempData["TaxSettingID"] != null && Int64.TryParse(TempData["TaxSettingID"].ToString(), out Id) && taxsetting.ID == Id)
				{
					...
					{
					log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} updated tax settings.");
						return Json(new
						{
							success = true,
							url = "/Admin/TaxSetting/Index",
		...
		public ActionResult Activate(long? id)
		{
			...
			if (!(bool)taxsetting.IsActive)
			{
				...
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} acivated tax settings.");
			}
			else
			{
				taxsetting.IsActive = false;
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} deacivated tax settings.");
			}
			string message = string.Empty;
			if (_taxsettingService.UpdateTaxSetting(ref taxsetting, ref message))
			{
		...
		public ActionResult DeleteConfirmed(long id)
		{
			...
			{
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} deleted tax settings.");
