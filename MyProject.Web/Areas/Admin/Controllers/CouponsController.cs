    ...
    public class CouponController : Controller
    {
                ...
                if (_couponService.CreateCoupon(ref coupon, ref message))
                {
                    if (coupon.IsOpenToAll.HasValue && coupon.IsOpenToAll.Value)
                    {
                        ...
                        if (_notificationService.CreateNotification(not, ref message))
                        {
                            ...
                            //{
                            //    var tokens = _customerSessionService.GetCustomerSessionFirebaseTokens((long)notRec.ReceiverID);
                    ...
                    log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} coupon created {coupon.Name}.");
                    return Json(new
                    {
                        success = true,
                        ...
                        data = new
                        {
                            ...
                            IsOpenToAll = coupon.IsOpenToAll
                        }
                    });
                }
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (TempData["CouponID"] != null && Int64.TryParse(TempData["CouponID"].ToString(), out Id) && coupon.ID == Id)
                {
                    ...
                    if (_couponService.UpdateCoupon(ref coupon, ref message, ref isNotifyRequired))
                    {
                        ...
                        log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} coupon updated {coupon.Name}.");
                        return Json(new
                        {
                            success = true,
                            url = "/Admin/Coupon/Index",
        ...
        public ActionResult Activate(long? id)
        {
            ...
            if (_couponService.UpdateCoupon(ref coupon, ref message, ref isNitifyRequired))
            {
                ...
                log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} coupon {((bool)coupon.IsActive ? "activated" : "deactivated")} {coupon.Name}.");
                return Json(new
                {
                    success = true,
        ...
        public ActionResult DeleteConfirmed(long id)
        {
            ...
            {
                Coupon coupon = _couponService.GetCoupon((Int16)id);
                log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} coupon deleted {coupon.Name}.");
                    ...
                    if (filename.EndsWith(".xlsx"))
                    {
                        ...
                        try
                        {
                            ...
                            foreach (var item in tenants)
                            {
                                ...
                                {
                                    if (_couponService.PostExcelData(item.Name, item.CouponCode, item.Frequency, item.MaxRedumption, item.Type, item.Value, item.MaxAmount, item.Expiry, item.IsOpenToAll == "Yes" ? true : false))
                                    {
                                    }
                                    else
                                    {
                        ...
                        log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} coupon bulk uploaded.");
                        return RedirectToAction("Index");
                    }
        ...
        public ActionResult SaveCustomerCoupons(long CustomerID, long CouponsID)
        {
            ...
            customerCoupon.CouponID = CouponsID;

            if (_customerCouponsService.CreateCustomerCoupon(customerCoupon, ref message))
            {
                var coupon = _couponService.GetCoupon((long)customerCoupon.CouponID);
                Notification not = new Notification();
                ...
                if (_notificationService.CreateNotification(not, ref message))
                {
                    ...
                    {
                        //var tokens = _customerSessionService.GetCustomerSessionFirebaseTokens((long)customerCoupon.CustomerID);
                        //var response = PushNotification.SendPushNotification(tokens, not.Title, not.Description, new
                        //{
                        //    Module = "Coupon",
                        //    RecordID = coupon.ID,
                ...
                log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} customer coupon Saved {coupon.Name}.");
                return Json(new
                {
                    success = true,
        ...
        public ActionResult DeleteCustomerCoupons(long CustomerID, long CouponsID)
        {
            ...
            try
            {
                ...
                {
                    log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} customer coupon deleted {coupon.Coupon.Name}.");
                    return Json(new
                    {
                        success = true,
