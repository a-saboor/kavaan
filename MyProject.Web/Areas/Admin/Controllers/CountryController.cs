	...
	public class CountryController : Controller
	{
		...
		public ActionResult Create(Country country)
		{
			...
			if (ModelState.IsValid)
			{
				...
				{
					log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} created country {country.Name}.");
					return Json(new
					{
						success = true,
		...
		public ActionResult Edit(Country country)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (TempData["CountryID"] != null && Int64.TryParse(TempData["CountryID"].ToString(), out Id) && country.ID == Id)
				{
					...
					{
						log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} updated country {country.Name}.");
						return Json(new
						{
							success = true,
							url = "/Admin/Country/Index",
		...
		public ActionResult Activate(long? id)
		{
			...
			if (_countryService.UpdateCountry(ref country, ref message))
			{
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} {((bool)country.IsActive ? "activated" : "deactivated")} {country.Name}.");
		...
		public ActionResult DeleteConfirmed(long id)
		{
			...
			{
				Country country = _countryService.GetCountry((Int16)id);
				log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} deleted country {country.Name}.");
				...
				if (FileUpload.ContentType == "application/vnd.ms-excel" || FileUpload.ContentType == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
				{
					...
					if (filename.EndsWith(".xlsx"))
					{
						...
						try
						{
							...
							log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} Bulk Uploaded country.");
