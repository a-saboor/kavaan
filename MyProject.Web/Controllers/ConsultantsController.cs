...
namespace MyProject.Web.Controllers{
	public class ConsultantsController : Controller
	{
		...
		[HttpPost]
		[Route("consultants/filters", Name = "consultants/filters")]
		public ActionResult Index(FilterViewModel filters, string culture = "en-ae")
		{
			...
			{
				string ImageServer = "";
				var consultants = _consultantService.GetFilteredConsultants(filters.search, filters.pageSize, filters.pageNumber, filters.sortBy, lang, ImageServer);
				return Json(new
				{
					success = true,
					message = "Data recieved successfully!",
					data = consultants.Select(x => new
					{
						x.ID,
						x.Date,
						x.Name,
		...
		[HttpGet]
		[Route("consultants/{id}", Name = "consultants/{id}")]
		public ActionResult Details(long id)
		{
			var consultant = _consultantService.Edit(id);//get single consultant
		...
		[HttpGet]
		[Route("consultants/get-all", Name = "consultants/get-all")]
		public ActionResult GetAll(string culture = "en-ae")
		{
			...
			{
				string ImageServer = "";
				var consultants = _consultantService.GetAll().Where(x => x.IsActive == true && x.IsDeleted == false).OrderByDescending(x => x.ID).Take(8);
