    ...
    public class CustomerLoyaltySettingController : Controller
    {
        ...
        public ActionResult Create(CustomerLoyaltySetting data)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} created customer loyalty setting {data.CustomerType}.");
                    return Json(new
                    {
                        success = true,
                        ...
                        data = new
                        {
                            Date = data.CreatedOn.Value.ToString(MyProject.Web.Helpers.CustomHelper.GetDateFormat),
        ...
        public ActionResult Edit(CustomerLoyaltySetting data)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (TempData["SettingID"] != null && Int64.TryParse(TempData["SettingID"].ToString(), out Id) && data.ID == Id)
                {
                    ...
                    {
                        log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} updated customer loyalty setting {data.CustomerType}.");
                        return Json(new
                        {
                            success = true,
        ...
        public ActionResult Activate(long? id)
        {
            ...
            if (_CustomerLoyaltySettingService.UpdateCustomerLoyaltySettings(ref Settings, ref message))
            {
                ...
                log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} {((bool)Settings.IsActive ? "activated" : "deactivated")} customer loyalty setting {Settings.CustomerType}.");
                return Json(new
                {
                    success = true,
                    ...
                    data = new
                    {
                        Date = Settings.CreatedOn.Value.ToString(MyProject.Web.Helpers.CustomHelper.GetDateFormat),
        ...
        {
            string message = string.Empty;
            CustomerLoyaltySetting cls = _CustomerLoyaltySettingService.GetSettings((Int16)id);
            if (_CustomerLoyaltySettingService.DeleteCustomerLoyaltySettig((Int16)id, ref message))
            {
                log.Info($"{AdminSessionHelper.UserName} | {AdminSessionHelper.Email} deleted customer loyalty setting {cls.CustomerType}.");
