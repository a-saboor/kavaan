...
$(document).ready(function () {
	...
	$('#ArticleSearch').change(function () {
		if ($('#ArticleSearch').val()) {
			$('#consultant_section .main-div').empty();
			pg = 1;
			GetFilterValues();
			GetArticles();
		}
	...
	$('#consultant_section .see-more').click(function () {
		//if (pg < totalPages) {
		pg++;
		$('#consultant_section .see-more').hide();
		$(".filter-loader").show();
		GetFilterValues();
		GetArticles();
		//}
...
function BindGridArticles(data) {
	...
	$('#consultant_section .main-div').empty().show();

	$.each(data, function (k, v) {

		$('#consultant_section .main-div').append(`
	...
	if (data && data.length >= PageSize) {
		$("#consultant_section .see-more").fadeIn();
	} else {
		$("#consultant_section .see-more").fadeOut();
	}

	if ($('#consultant_section .main-div').html().length == 0) {
		$("html, body").animate({ scrollTop: 0 }, 1000);
		$('#consultant_section .no-more').fadeIn();
		$("#consultant_section .see-more").fadeOut();
	}
}
