...
$(document).ready(function () {
	...
	$('#partners_section .main-div').empty().html(return_loading_section(0));
	...
	$('#ArticleSearch').change(function () {
		if ($('#ArticleSearch').val()) {
			$('#partners_section .main-div').empty();
			pg = 1;
			GetFilterValues();
			GetArticles();
		}
	...
	$('#partners_section .see-more').click(function () {
		//if (pg < totalPages) {
		pg++;
		$('#partners_section .see-more').hide();
		$(".filter-loader").show();
		GetFilterValues();
		GetArticles();
		//}
...
function BindGridArticles(data) {
	...
	$.each(data, function (k, v) {
		...
		$('#partners_section .main-div').append(`
			...
			<a class="flex flex-row pb-4 w-full sm:w-[49%]">
				<div class="bg-black/5 w-[25%] flex justify-center items-center rounded-l-lg drop-shadow-lg">
					<img src="${v.Image}" alt="" class="w-10 h-10 xl:w-12 xl:h-12 rounded">
				</div>
	...
	if (data && data.length >= PageSize) {
		$("#partners_section .see-more").fadeIn();
	} else {
		$("#partners_section .see-more").fadeOut();
	}

	if ($('#partners_section .main-div section')) {
		$('#partners_section .main-div section').remove();
	}

	if ($('#partners_section .main-div').html().length == 0) {
		$("html, body").animate({ scrollTop: 0 }, 1000);
		$('#partners_section .no-more').fadeIn();
		$("#partners_section .see-more").fadeOut();
	}
}
