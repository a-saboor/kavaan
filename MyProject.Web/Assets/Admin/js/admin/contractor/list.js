			...
			columnDefs: [
				{
					...
					render: function (data, type, full, meta) {
						...
						if (full[2] === "admin@KAVAAN.com") {
							actions += '<button type="button" class="btn btn-secondary btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/contractor/Details/' + data[1] + '\')" title="View">' +
							...
							actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/contractor/Edit/' + data[1] + '\',true)">' +
								'<i class="fa fa-pen"></i>' +
								'</button> ' +
								'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/contractor/Details/' + data[1] + '\')" title="View">' +
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						...
						return '<div class="d-flex align-items-center">' +
							...
							'<div class="symbol-label" style="background-image: url(\'' + splitData[1] + '\')"></div>' +
							'</div>' +
							'<div>' +
							'<a href="javascript:void(0);" class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-lg">' + splitData[0] + '</a>' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/contractor/Delete/' + record,
				type: 'POST',
				data: {
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/contractor/Activate/' + record,
				type: 'Get',
