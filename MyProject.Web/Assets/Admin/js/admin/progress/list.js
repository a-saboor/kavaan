			...
			columnDefs: [
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						...
						actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Progress/Edit/' + data[1] + '\',true)">' +
							'<i class="fa fa-pen"></i>' +
							'</button> ' +
							'<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Admin/Progress/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
	...
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/Progress/Delete/' + record,
				type: 'POST',
				data: {
				...
				success: function (result) {
					if (result.success != undefined) {
						if (result.success) {
							...
							toastr.success('Progress deleted successfully ...');
...
function Activate(element, record) {
	swal.fire({
		...
		if (result.value) {
			...
			$.ajax({
				url: '/Admin/Progress/Activate/' + record,
				type: 'Get',
