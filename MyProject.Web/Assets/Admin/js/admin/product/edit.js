...
jQuery(document).ready(function () {
	...
	$('#kt_image_product_image #Image').change(function () {
		...
		if (files.length > 0) {
			...
			$.ajax({
				url: "/Admin/Product/Thumbnail/" + GetURLParameter(),
				type: "POST",
				processData: false,
				contentType: false,
				data: data,
			...
			if (response.success) {
				...
				$('.product-category input[type="checkbox"]').change(function () {
					...
					if ($(chkcategory).prop('checked')) {
						...
						$.ajax({
							url: '/Admin/ProductCategory/Delete/' + $(chkcategory).attr('productCategoryId'),
							type: 'POST',
							data: {
								"__RequestVerificationToken":
									$("input[name=__RequestVerificationToken]").val()
...
function BindProductVariation(id, data, productVariation, sku) {
	...
	template += '						</div>';
	template += '						<input type="file" name="ProductVariationGalleryImages" value="" multiple style="display:none" onchange="UploadProductVariationGalleryImages(this,' + id + ')"/>';
	template += '					</div>';
	template += '				</div>';
...
function DeleteVariation(elem, id) {
	...
	$.ajax({
		url: '/Admin/ProductVariation/Delete/' + id,
		type: 'POST',
		data: {
			"__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val()
		},
		success: function (response) {
			if (response.success) {
				...
				$.ajax({
					url: '/Admin/ProductVariationAttribute/DeleteAll/' + id,
					type: 'POST',
					data: {
						"__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val()
					},
...
function UpdateVaraitionAttributes(elem) {
	...
	if ($('.variation-attributes-form select').length == $('.variation-attributes-form select option[value!=""]:checked').length) {
		...
		$.ajax({
			url: '/Admin/ProductVariationAttribute/Update/',
			type: 'POST',
			data: {
				"__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val(),
...
function ChangeProductVariationThumbnail(elem, productVariationId) {
	...
	if (files.length > 0) {
		...
		$(elem).closest('.image-input').find('label[data-action="change"]').addClass('spinner spinner-dark spinner-center spinner-sm').prop('disabled', true);
		$.ajax({
			url: "/Admin/ProductVariation/Thumbnail/" + productVariationId,
			type: "POST",
			processData: false,
			contentType: false,
			data: data,
...
function UploadProductVariationGalleryImages(elem, productVariationId) {
	...
	if (files.length > 0) {
		...
		$.ajax({
			url: "/Admin/ProductVariationImages/Create/" + productVariationId,
			type: "POST",
			processData: false,
			contentType: false,
			data: data,
...
function BindProductVariationImages(productVariationId) {
	$.ajax({
		url: '/Admin/ProductVariationImages/GetProductVariationImages/' + productVariationId,
		type: 'GET',
		success: function (response) {
			if (response.success) {
...
function DeleteProductVariationGalleryImage(elem, record) {
	...
	$(elem).find('i').hide();
	$.ajax({
		url: '/Admin/ProductVariationImages/Delete/' + record,
		type: 'POST',
		data: {
			"__RequestVerificationToken":
				$("input[name=__RequestVerificationToken]").val()
...
var KTTagifyProductVariationAttribute = function (id) {
	...
	var demo1 = function (id) {
		...
		//tagify.on('add', function (e) {
		//	$.ajax({
		//		url: '/Admin/ProductVariationAttribute/Create/',
		//		type: 'POST',
		//		data: {
		//			"__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val(),
		//			productAttribute: {
		...
		//	if (e.detail.data.id) {
		//		$.ajax({
		//			url: '/Admin/ProductVariationAttribute/Delete/' + e.detail.data.id,
		//			type: 'POST',
		//			data: {
		//				"__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val()
		//			},
		...
		//	} else {
		//		$.ajax({
		//			url: '/Admin/ProductVariationAttribute/DeleteValue/',
		//			type: 'POST',
		//			data: {
		//				"__RequestVerificationToken": $("input[name=__RequestVerificationToken]").val(),
		//				productAttribute: {
