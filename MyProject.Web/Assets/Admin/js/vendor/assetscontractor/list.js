            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (full[2] === "admin@KAVAAN.com") {
                            actions += '<button type="button" class="btn btn-secondary btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/AssetsContractor/Details/' + data[1] + '\')" title="View">' +
                            ...
                            actions += '<a  class="btn btn-bg-secondary btn-icon btn-sm mr-1" href="/Vendor/AssetsContractor/Edit/' + data[1] + '">' +
                                '<i class="fa fa-pen"></i>' +
                                '</a> ' +
                                '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/AssetsContractor/Details/' + data[1] + '\')" title="View">' +
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var assetcontractor = data.split('|');
                        return '<div class="d-flex align-items-center">' +
                            '<div>' +
                            '<a href="javascript:void(0);" class="text-dark-75 font-weight-bolder mb-1 font-size-lg">' + assetcontractor[0] + '</a><br>' +
                            '<a class="text-dark-75 text-hover-primary mb-1 font-size-lg">' + assetcontractor[1] + '</a>' +
                            '</div>' +
                            '</div>';
                    },
                },
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Vendor/AssetsContractor/Delete/' + record,
                type: 'POST',
                data: {
                ...
                success: function (result) {
                    if (result.success != undefined) {
                        if (result.success) {
                            ...
                            $.each($('td input[value="' + Number(record) + '"]'), function (k, elem) {
                                debugger
...
//			$.ajax({
//				url: '/Vendor/blog/Activate/' + record,
//				type: 'Get',
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Vendor/AssetsContractor/Activate/' + record,
                type: 'Get',
                success: function (response) {
                    if (response.success) {
                        toastr.success(response.message);
