                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (full[2] === "admin@KAVAAN.com") {
                            actions += '<button type="button" class="btn btn-secondary btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/AssetsCategory/Details/' + data[1] + '\')" title="View">' +
                            ...
                            actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/AssetsCategory/Edit/' + data[1] + '\',true)">' +
                                '<i class="fa fa-pen"></i>' +
                                '</button> ' +
                                '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Vendor/AssetsCategory/Details/' + data[1] + '\')" title="View">' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Vendor/AssetsCategory/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Vendor/AssetsCategory/Activate/' + record,
                type: 'Get',
                success: function (response) {
                    if (response.success) {
                        toastr.success(response.message);
                        ...
                        if ($(element).text() !== ' Activate') {
                            ...
                            if ($('td input[value="' + Number(record) + '"]').length > 0) {
                                setTimeout(function () {
                                    location.reload();
                                }, 1000);
                            }
